...
export default function Learn() {
  const lead = ARTICLES.find((a) => a.pinned) ?? ARTICLES[0]
    ...
    <main className="mx-auto max-w-6xl px-5 pb-24">
      ...
      >
        <p className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground group-hover:text-background/70 mb-2">
          {lead.pinned ? 'pinned' : 'lead story'} · {lead.minutes} min
        </p>
      ...
      <div className="grid sm:grid-cols-3 border-b-2 border-foreground">
        {rest.map((a) => (
          <Link
            key={a.slug}
            to={`/learn/${a.slug}`}
            className="group block py-6 px-2 sm:px-5 hover:bg-foreground hover:text-background transition-colors border-t-2 border-foreground"
          >
            <p className="text-[10px] uppercase tracking-[0.25em] text-muted-foreground group-hover:text-background/70 mb-2">
              {a.minutes} min
            </p>
